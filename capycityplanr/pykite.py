...
def create_dataset(kite_path,data):
    ...
    p = subprocess.Popen([kite_path,'create',kite_class,'-s',schema],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    out,err = p.communicate()
    if err:
        logging.error(err)
    else:
...
def import_data(kite_path,data):
    ...
    p = subprocess.Popen([kite_path,'csv-import',csv_path,kite_class],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    out,err = p.communicate()
    if err:
        logging.error(err)
    else:
